...
async function zadarmaRequest(method, path, params = {}) {
  ...
  // Zadarma HMAC-SHA1 Signatur
  const crypto = require("crypto");
  const paramStr = new URLSearchParams(params).toString();
  const md5 = crypto.createHash("md5").update(paramStr).digest("hex");
  const signStr = path + paramStr + md5;
  const sign = Buffer.from(
    crypto.createHmac("sha1", secret).update(signStr).digest()
  ).toString("base64");

  const url = method === "GET"
    ? `${ZADARMA_BASE}${path}?${paramStr}`
    : `${ZADARMA_BASE}${path}`;
...
app.get("/api/zadarma/calls", async (req, res) => {
  ...
  try {
    ...
    const fmt = (d) => d.toISOString().split("T")[0] + " 00:00:00";
...
app.post("/webhook/zadarma", (req, res) => {
  ...
  // Anruf-Event verarbeiten
  if (event.event === "NOTIFY_END" || event.event === "NOTIFY_ANSWER") {
    console.log(`📋 Anruf: ${event.caller_id} → ${event.called_did} · ${event.duration}s · ${event.disposition}`);
  }
});
