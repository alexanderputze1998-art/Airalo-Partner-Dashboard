...
app.post("/api/refund", async (req, res) => {
  ...
  const { order_id, reason, comment, customer_email, shopify_order } = req.body;
  if (!order_id) return res.status(400).json({ error: "order_id fehlt" });

  try {
    console.log(`↩ Refund beantragt für Order #${order_id}: ${reason}`);
    const data = await airaloRequest("POST", `/v2/orders/${order_id}/refund`, {
