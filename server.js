...
function createMailTransport() {
  return nodemailer.createTransport({
    ...
    secure: true, // Port 465 = SSL immer true
    auth: {
...
app.post("/api/email/send", async (req, res) => {
  ...
  try {
    const transporter = createMailTransport();
    await transporter.sendMail({
      from: `"${process.env.SHOP_NAME || "For You eSIM"}" <${process.env.SMTP_USER}>`,
      to,
      subject,
      text: body,
      html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#333">
        ...
        <p style="color:#999;font-size:12px">${process.env.SHOP_NAME || "For You eSIM"} · ${process.env.SUPPORT_EMAIL || process.env.SMTP_USER}</p>
      </div>`,
    });
    console.log(`📧 Dashboard E-Mail gesendet an: ${to}`);
          ...
          fetch.on("message", (msg) => {
            ...
            msg.once("end", () => {
              try {
                ...
                const subject = subjectMatch ? subjectMatch[1].trim() : "(kein Betreff)";
                const date = dateMatch ? new Date(dateMatch[1]).toLocaleDateString("de") : "–";
                const preview = text.replace(/=\r?\n/g, "").replace(/[^\x20-\x7E\u00C0-\u017E]/g, " ").substring(0, 120).trim();
                results.push({ from, subject, date, preview, read: true });
              } catch(e) {}
            });
          });
