...
app.get("/api/gocardless/payments", async (req, res) => {
  ...
  try {
    ...
    const result = await client.payments.list({ limit: 50 });
...
app.get("/api/gocardless/mandates", async (req, res) => {
  ...
  try {
    const client = getGCClient();
    const result = await client.mandates.list({ limit: 50 });
    res.json({ mandates: result.mandates || [] });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/gocardless/billing-request", async (req, res) => {
  ...
  try {
    ...
    const brPayload = {
      payment_request: { amount, currency: currency || 'EUR', description: description || 'eSIM Zahlung' },
    };
    if (collect_mandate) {
      brPayload.mandate_request = { currency: currency || 'EUR', scheme: 'sepa_core' };
    }
    const billingRequest = await client.billingRequests.create(brPayload, uuidv4());
    ...
    const flow = await client.billingRequestFlows.create({
      ...
      links: { billing_request: billingRequest.billing_request.id },
    }, uuidv4());

    const link = flow.billing_request_flow.authorisation_url;

    if (send_email && link) {
      const html = `<div style="font-family:sans-serif;max-width:580px;margin:0 auto;padding:20px">
        <p>Hallo ${given_name || ''},</p>
        <p>bitte begleichen Sie <strong>€${(amount/100).toFixed(2)}</strong> für <strong>${description}</strong>.</p>
        <div style="text-align:center;margin:24px 0">
          <a href="${link}" style="background:linear-gradient(135deg,#00cc55,#008833);color:#fff;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px;display:inline-block">🔒 Sicher bezahlen</a>
        </div>
        <p style="color:#999;font-size:11px;text-align:center">${process.env.SHOP_NAME || "For You eSIM"}</p>
      </div>`;
      await sendViaResend(email, `💳 Zahlungsanforderung – ${description}`, html);
    }

    res.json({ billing_request: billingRequest.billing_request, billing_request_flow: flow.billing_request_flow });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/gocardless/payment", async (req, res) => {
  ...
  try {
    ...
    const payment = await client.payments.create({
      ...
      description: description || 'eSIM Zahlung',
      links: { mandate: mandate_id },
    }, uuidv4());
    console.log(`💳 GoCardless Zahlung: ${payment.payment.id}`);
    res.json({ payment: payment.payment });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (event.resource_type === 'payments') {
        pushNotification({ type: 'payment', status: event.action, id: event.links?.payment });
      }
    }
