...
// ============================================================
//  GOCARDLESS API – Direkte HTTP-Calls
// ============================================================

async function gcRequest(method, path, body = null) {
  const token = process.env.GOCARDLESS_TOKEN;
  if (!token) throw new Error("GOCARDLESS_TOKEN nicht gesetzt");
...
app.get("/api/gocardless/payments", async (req, res) => {
  const apiKey = req.headers["x-api-key"];
  if (apiKey !== process.env.DASHBOARD_API_KEY) return res.status(401).json({ error: "Unauthorized" });
  try {
    const data = await gcRequest('GET', '/payments?limit=50');
    res.json({ payments: data.payments || [] });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

app.get("/api/gocardless/mandates", async (req, res) => {
  const apiKey = req.headers["x-api-key"];
  if (apiKey !== process.env.DASHBOARD_API_KEY) return res.status(401).json({ error: "Unauthorized" });
  try {
    const data = await gcRequest('GET', '/mandates?limit=50');
    res.json({ mandates: data.mandates || [] });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

app.get("/api/gocardless/customers", async (req, res) => {
  const apiKey = req.headers["x-api-key"];
  if (apiKey !== process.env.DASHBOARD_API_KEY) return res.status(401).json({ error: "Unauthorized" });
  try {
    const data = await gcRequest('GET', '/customers?limit=50');
    res.json({ customers: data.customers || [] });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

app.post("/api/gocardless/billing-request", async (req, res) => {
  const apiKey = req.headers["x-api-key"];
  if (apiKey !== process.env.DASHBOARD_API_KEY) return res.status(401).json({ error: "Unauthorized" });
  const { email, given_name, family_name, amount, currency, description, collect_mandate, send_email } = req.body;
  if (!email || !amount) return res.status(400).json({ error: "email und amount fehlen" });
  try {
    const brBody = { billing_requests: { payment_request: { amount: parseInt(amount), currency: currency || 'EUR', description: description || 'eSIM Zahlung' } } };
    if (collect_mandate) brBody.billing_requests.mandate_request = { scheme: 'sepa_core' };
    const br = await gcRequest('POST', '/billing_requests', brBody);
    const brId = br.billing_requests?.id;
    if (!brId) throw new Error('Billing Request ID nicht erhalten');

    const flow = await gcRequest('POST', '/billing_request_flows', {
      billing_request_flows: {
        redirect_uri: 'https://fouryouesim.de',
        exit_uri: 'https://fouryouesim.de',
        prefilled_customer: { email, given_name: given_name || '', family_name: family_name || '' },
        links: { billing_request: brId },
      }
    });
    const link = flow.billing_request_flows?.authorisation_url;

    if (send_email && link) {
      const html = `<div style="font-family:sans-serif;max-width:580px;margin:0 auto;padding:20px"><p>Hallo ${given_name || ''},</p><p>bitte begleichen Sie <strong>€${(amount/100).toFixed(2)}</strong> für <strong>${description}</strong>.</p><div style="text-align:center;margin:24px 0"><a href="${link}" style="background:#00cc55;color:#fff;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:700;display:inline-block">🔒 Sicher bezahlen</a></div><p style="color:#999;font-size:11px;text-align:center">${process.env.SHOP_NAME || "For You eSIM"}</p></div>`;
      await sendViaResend(email, `💳 Zahlungsanforderung – ${description}`, html);
    }

    res.json({ billing_request: br.billing_requests, billing_request_flow: flow.billing_request_flows });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

app.post("/api/gocardless/payment", async (req, res) => {
  const apiKey = req.headers["x-api-key"];
  if (apiKey !== process.env.DASHBOARD_API_KEY) return res.status(401).json({ error: "Unauthorized" });
  const { mandate_id, amount, currency, description } = req.body;
  if (!mandate_id || !amount) return res.status(400).json({ error: "mandate_id und amount fehlen" });
  try {
    const data = await gcRequest('POST', '/payments', { payments: { amount: parseInt(amount), currency: currency || 'EUR', description: description || 'eSIM Zahlung', links: { mandate: mandate_id } } });
    console.log(`💳 GoCardless Zahlung: ${data.payments?.id}`);
    res.json({ payment: data.payments });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

app.post("/webhook/gocardless", express.raw({ type: 'application/json' }), (req, res) => {
  res.status(200).send('OK');
  try {
    const events = JSON.parse(req.body).events || [];
    for (const event of events) {
      console.log(`💳 GoCardless: ${event.resource_type} ${event.action}`);
      if (event.resource_type === 'payments') pushNotification({ type: 'payment', status: event.action, id: event.links?.payment });
    }
  } catch(e) { console.error('GoCardless Webhook Fehler:', e.message); }
});
